...
export async function getStoredKeypair(): Promise<DeviceKeypair | null> {
	const stored = (await chrome.storage.local.get(DEVICE_KEYPAIR_KEY))[DEVICE_KEYPAIR_KEY] as
		| DeviceKeypair
		| undefined;
...
export async function storeKeypair(kp: DeviceKeypair): Promise<void> {
	await chrome.storage.local.set({ [DEVICE_KEYPAIR_KEY]: kp });
}
...
export async function getStoredGroup(): Promise<GroupConfig | null> {
	const g = (await chrome.storage.local.get(GROUP_KEY))[GROUP_KEY] as GroupConfig | undefined;
...
export async function getStoredRelay(): Promise<string> {
	const r = (await chrome.storage.local.get(RELAY_KEY))[RELAY_KEY];
